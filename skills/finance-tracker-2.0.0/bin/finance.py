...
# ── Ensure lib/ is on sys.path (multiple strategies) ──────────────────
# Strategy 1: relative to this script file
_script_dir = os.path.dirname(os.path.abspath(__file__))
_lib_dir = os.path.normpath(os.path.join(_script_dir, os.pardir, "lib"))

# Strategy 2: relative to cwd (execute_command.py sets cwd to skill root)
_cwd_lib = os.path.normpath(os.path.join(os.getcwd(), "lib"))

# Strategy 3: PYTHONPATH (execute_command.py injects this)
_pypath_dirs = [
    d.strip()
...
# Insert all valid lib paths at the front of sys.path
for _candidate in [_lib_dir, _cwd_lib] + _pypath_dirs:
    if _candidate and os.path.isdir(_candidate) and _candidate not in sys.path:
        sys.path.insert(0, _candidate)

# Strategy 4: last resort — if none of the above worked, try to find lib/
# by walking up from __file__ looking for a directory containing categories.py
if not any(os.path.isfile(os.path.join(p, "categories.py")) for p in sys.path):
    _search = os.path.abspath(__file__)
    for _ in range(5):
        _search = os.path.dirname(_search)
        _maybe_lib = os.path.join(_search, "lib")
        if os.path.isfile(os.path.join(_maybe_lib, "categories.py")):
            sys.path.insert(0, _maybe_lib)
            break
...
def cmd_report(args):
    ...
    print(generate_report(period, extra_args=extra_args))
    return 0
...
REPORT PERIODS:
  ...
  YYYY-MM (e.g. 2026-03), YYYY-MM-DD (specific day)
...
EXAMPLES:
  ...
  finance report 2026-04-01 2026-04-15
